# 📡 Session Tracker — LAN Session Monitor
...
```bash
git clone https://github.com/YOUR_USERNAME/session-tracker.git
cd session-tracker
```
...
- Port **2003** must not be blocked by a firewall. If needed, allow it:
  - **Windows:** `netsh advfirewall firewall add rule name="SessionTracker" dir=in action=allow protocol=UDP localport=2003`
